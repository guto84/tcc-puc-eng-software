create table tb_category (group_id uuid, id uuid not null, name varchar(255) not null, primary key (id));
create table tb_company (id uuid not null, name varchar(255), url varchar(255) unique, primary key (id));
create table tb_configuration (maximum integer not null, minimum integer not null, category_id uuid, id uuid not null, name varchar(255) not null, primary key (id));
create table tb_configuration_item (price float(53) not null, configuration_id uuid, id uuid not null, description varchar(255), name varchar(255) not null, primary key (id));
create table tb_group (company_id uuid, id uuid not null, name varchar(255) not null, primary key (id));
create table tb_order (status smallint check (status between 0 and 4), created_at timestamp(6) not null, updated_at timestamp(6) not null, company_id uuid, id uuid not null, address varchar(255) not null, address_complement varchar(255), address_number varchar(255) not null, district varchar(255) not null, name varchar(255) not null, phone varchar(255) not null, zipcode varchar(255) not null, primary key (id));
create table tb_order_configuration (quantity float(53), configuration_item_id uuid not null, order_item_id uuid not null, primary key (configuration_item_id, order_item_id));
create table tb_order_item (quantity float(53) not null, id uuid not null, order_id uuid, product_id uuid, primary key (id));
create table tb_product (price float(53), category_id uuid, id uuid not null, description varchar(255), name varchar(255) not null, primary key (id));
create table tb_role (id uuid not null, authority varchar(255), primary key (id));
create table tb_user (company_id uuid, id uuid not null, email varchar(255) not null unique, name varchar(255) not null, password varchar(255) not null, primary key (id));
create table tb_user_role (role_id uuid not null, user_id uuid not null, primary key (role_id, user_id));
alter table if exists tb_category add constraint FKsljji95y6ki4o7ybcpcc4ha7 foreign key (group_id) references tb_group;
alter table if exists tb_configuration add constraint FKdfsrhjotnnibme1l7570phkcs foreign key (category_id) references tb_category;
alter table if exists tb_configuration_item add constraint FKl1jmbm8qoyxmhap92859rewo2 foreign key (configuration_id) references tb_configuration;
alter table if exists tb_group add constraint FKgvlalck118knlk873mto677r8 foreign key (company_id) references tb_company;
alter table if exists tb_order add constraint FKcb8krrxlgw353os0vnjdjphdh foreign key (company_id) references tb_company;
alter table if exists tb_order_configuration add constraint FKfpl14le03wrrvoc5bwbdltgre foreign key (configuration_item_id) references tb_configuration_item;
alter table if exists tb_order_configuration add constraint FK56whkx6mihe5h5u0nnfixw4hp foreign key (order_item_id) references tb_order_item;
...
alter table if exists tb_user add constraint FKib3edn6o0l2nlr2vlpl48idfs foreign key (company_id) references tb_company;
